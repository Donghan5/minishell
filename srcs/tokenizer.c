...
/*   Updated: 2024/10/21 19:27:31 by pzinurov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	copy_quoted(char **splitted, char const **s, int *quoted, int *i_j)
{
	...
	if (**s != *quoted)
		return (handle_errors_tokens(splitted, \
			NULL, "minishell: unexpected EOF \
while looking for matching `'' or `\"'\n"));
	else
	{
		splitted[i_j[0]][i_j[1]++] = *(*s);
		(*s)++;
...
char	**tokenizer(char const *s)
{
	...
	if (tokens == -1)
		return (smart_print_err("minishell: unexpected \
EOF while looking for matching `'' or `\"'\n"), NULL);
